#!/usr/bin/env python3
"""Build the 101-class (100 tools + no_tool) tool-routing dataset.

Same source as the 25-class run: glaiveai/glaive-function-calling-v2
(apache-2.0), read from the local raw dump (same rows, same order as the
HF streaming source used by scripts/make_data.py).

Identical pipeline to scripts/make_data.py: same turn splitting, same
SYNONYMS, same dedupe, same per-class caps (700 tool / 2000 no_tool), same
stratified 70/15/15 split, same seed (42). Only TOP_K differs
(100 canonical tools -> 101 classes incl. no_tool; cutoff = 74 examples,
the count of the rank-100 canonical tool, so every class has >= ~10
examples in each of val/test).

Output (into data/scale100/): data_train.jsonl, data_val.jsonl,
        data_test.jsonl, label_map.json, data_stats.json
"""
import json, re, collections, random, os
from sklearn.model_selection import train_test_split

RAW_DUMP = "data/glaive/raw/glaive-function-calling-v2.json"
TOP_K = 100
PER_TOOL_CAP = 700
NOTOOL_CAP = 2000
SEED = 42
OUT_DIR = "data/scale100"  # run from repo root

# variant -> canonical tool name (same intent, different name in the wild)
# IDENTICAL to scripts/make_data.py
SYNONYMS = {
...
def extract_rows():
    items = json.load(open(RAW_DUMP))  # same rows/order as the HF streaming source
...
def main():
    ...
    print("top 100 after normalization:")
    for nm in top_tools:
