...
if [ -f exp/baseline_gen_fewshot_report.json ]; then echo "[stage2] few-shot OK"; else echo "[stage2] FEWSHOT REPORT MISSING"; fi
echo "[stage2] launching OOD generation (ollama/phi4)..."
$VENV scripts/ood/generate_ood.py > exp/ood/generate_ood.log 2>&1
echo "[stage2] ood exit=$? examples=$(wc -l < exp/ood/ood_challenge.jsonl 2>/dev/null)"
date > exp/ood/STAGE2_DONE
