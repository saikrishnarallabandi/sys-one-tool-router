...
CKPT = f"{BASE}/exp/checkpoints/qwen3-0.6b-tool-router-100/best"
EXP = f"{BASE}/exp/scale100"

def main():
    import os; os.makedirs(EXP, exist_ok=True)
    label_map = json.load(open(f"{BASE}/data/scale100/label_map.json"))["label2id"]
    id2label = {v: k for k, v in label_map.items()}
    no_tool_id = label_map["no_tool"]
    T = json.load(open(f"{EXP}/temperature.json"))["temperature"]
    ...
    json.dump(out, open(f"{EXP}/selective_report.json", "w"), indent=2)
