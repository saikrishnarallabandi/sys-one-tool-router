...
MAX_LEN = 128
OUT = "exp/checkpoints/qwen3-0.6b-tool-router"
...
def main():
    ...
    train_d = ListDataset(load_split("data/data_train.jsonl", tok))
    val_d = ListDataset(load_split("data/data_val.jsonl", tok))

    # class weights from the TRAIN split
    counts = np.bincount([r for r in
                          (json.loads(l)["label"] for l in open("data/data_train.jsonl"))],
