...
CKPT = f"{BASE}/checkpoints/qwen3-0.6b-tool-router/best"
...
def main():
    ...
    T = json.load(open(f"{BASE}/temperature.json"))["temperature"]
    items = [json.loads(l) for l in open(f"{BASE}/ood/ood_challenge.jsonl")]
    ...
    seen = set()
    for f in ["data_train.jsonl", "data_val.jsonl", "data_test.jsonl"]:
    ...
    json.dump(out, open(f"{BASE}/ood/ood_eval_report.json", "w"), indent=2)
