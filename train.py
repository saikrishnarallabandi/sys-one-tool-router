...
MAX_LEN = 128
OUT = "checkpoints/qwen3-0.6b-tool-router"
...
def main():
    ...
    train_d = ListDataset(load_split("data_train.jsonl", tok))
    val_d = ListDataset(load_split("data_val.jsonl", tok))

    # class weights from the TRAIN split
    counts = np.bincount([r for r in
                          (json.loads(l)["label"] for l in open("data_train.jsonl"))],
