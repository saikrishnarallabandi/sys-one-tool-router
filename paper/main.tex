...
\title{Sys One: Fast, Calibrated Tool-Call Routing in a Single Forward Pass}

\author{
  Sai Krishna Rallabandi \\
  \texttt{srallaba.research@gmail.com}
}
...
per decision on a GTX 1080 Ti, emitting per-class probabilities and calibrated
confidence as typed output. Our central finding concerns calibration:
temperature fitted on validation ($T=1.79$) exposes genuine overconfidence
and improves test NLL ($0.0310 \to 0.0261$) and Brier score
($0.0131 \to 0.0120$)---while a companion experiment on easier data collapsed
to $T=1.00$, hiding miscalibration entirely. Easy validation sets do not just
flatter accuracy; they conceal overconfidence. We release code, curated
...
\begin{itemize}
  ...
  \item A calibration finding with a practical moral. Temperature scaling
...
retrieval. We instead treat routing over a \emph{fixed, canonical} catalog as
classification, which is the right abstraction when the tool set is known in
advance---the common production case.
...
We use Qwen3-0.6B \citep{qwen2025report} with a sequence-classification head:
\begin{equation}
...
All experiments run on a single NVIDIA GTX 1080 Ti. Metrics: accuracy,
...
\begin{table}[t]
  \centering
  \caption{Test-set results ($n=1{,}641$, 25 classes). Temperature $T=1.7912$
  is fit on the validation split only.}
  \label{tab:main}
  \begin{tabular}{lcc}
    \toprule
    Metric & Uncalibrated ($T{=}1.0$) & Scaled ($T{=}1.791$) \\
    \midrule
    Accuracy & 0.9927 & 0.9927 \\
    Macro-F1 & 0.9894 & 0.9894 \\
    NLL & 0.0310 & \textbf{0.0261} \\
    Brier & 0.0131 & \textbf{0.0120} \\
    ECE (15 bins) & 0.0067 & 0.0077 \\
    \bottomrule
  \end{tabular}
\end{table}

Table~\ref{tab:main} summarizes the headline: \textbf{99.3\% accuracy, 0.989
macro-F1} on the held-out test set. Temperature scaling leaves accuracy and
...
roughly two orders of magnitude faster than an LLM-as-judge tool-selection
call (typically 2--30\,s), with typed output and no parsing step. Batching
...
probabilities with calibrated confidence. Beyond the numbers, the durable
lesson is about calibration methodology: temperature scaling fit on easy
validation data reported perfect calibration ($T=1.00$) that was an artifact
of the data, not a property of the model; the same procedure on
representative data exposed real overconfidence ($T=1.79$) and fixed it.
Calibrate against the distribution you will serve, or do not bother.
...
Code, datasets (\texttt{data\_train.jsonl} / \texttt{data\_val.jsonl} /
\texttt{data\_test.jsonl}), label map, fitted temperature, and the evaluation
report are in the public repository. \texttt{requirements.txt} pins the
exact dependency versions. Reference run: \texttt{python make\_data.py}
$\to$ \texttt{python train.py} ($\sim$20\,min on a GTX 1080 Ti) $\to$
