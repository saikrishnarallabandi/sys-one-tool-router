...
All experiments run on a single NVIDIA GTX 1080 Ti, except the Glaive-100
scale run, which was trained and evaluated on a rented RTX 3090 while the
...
GPU---compare with care). Macro-F1 is over the classes present
...
validation split only ($T=1.7912$ curated, $T=1.4063$ Glaive-100). The
generative baseline for the Glaive-100 setting is pending (---) and will
complete the row when run.}
  \label{tab:main}
