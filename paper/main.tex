...
\begin{table}[t]
  \centering
  \caption{Quality vs.\ latency on the test set ($n=1{,}641$, 25 classes).
  Generative baselines use the same base model (Qwen3-0.6B), plain-text prompts,
  greedy decoding ($\le$16 tokens), and no confidence output; parse failures
  count as wrong. Latency is mean per-example on one GTX 1080 Ti, fp16,
  including tokenization. Macro-F1 is over the 25 real classes with parse
  failures as errors (no invented 26th class). Temperature $T=1.7912$ is fit
  on the validation split only.}
  \label{tab:main}
  \begin{tabular}{lcccc}
    \toprule
    Method & Accuracy & Macro-F1 & Unparseable & Mean latency \\
    \midrule
    Sys One (single forward pass) & \textbf{0.9927} & \textbf{0.9894} & --- & \textbf{29.8\,ms} \\
    Generative, zero-shot & 0.3473 & 0.3675 & 28.1\% & 495.1\,ms \\
    Generative, 3-shot & 0.5119 & 0.5233 & 15.2\% & 500.6\,ms \\
    \bottomrule
  \end{tabular}
\end{table}
