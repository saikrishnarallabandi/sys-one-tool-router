...
\begin{table}[t]
  \centering
  \small
  \caption{Leaderboard: routing quality and latency by dataset and task. The
generative column is the stronger 3-shot baseline (same Qwen3-0.6B, plain-text
prompts, greedy decoding, $\le$16 tokens, no confidence output); parse failures
count as wrong (28.1\% zero-shot, 15.2\% 3-shot on the curated set). Latency is
mean per-example on one GTX 1080 Ti, fp16, including tokenization. Macro-F1 is
over the classes present in each split, with parse failures as errors. The BFCL
row is a frozen zero-shot transfer probe---its generative baseline is not yet
measured, and it ran on CPU (GPU unhealthy), so no latency is reported.
Temperature $T=1.7912$ is fit on the validation split only.}
  \label{tab:main}
  \begin{tabular}{lcc@{\hspace{1.2em}}cc@{\hspace{1.2em}}cc}
    ...
    BFCL mapped, frozen transfer ($n{=}69$) & --- & \textbf{0.913} & --- & \textbf{0.889} & --- & --- \\
    \bottomrule
  \end{tabular}
\end{table}
...
The second row is a different kind of test: a frozen zero-shot transfer probe.
We conservatively mapped 68 BFCL \citep{bfcl2024} functions onto our 25 classes
(69 single-function examples) and evaluated the frozen router with no
retraining. Accuracy is 91.3\% (macro-F1 0.889), and the abstention machinery
transfers with it: 98.4\% selective accuracy at 88.4\% coverage under the 0.9
confidence threshold. The generative column is empty because that baseline has
not yet been measured on BFCL.
