...
All experiments run on a single NVIDIA GTX 1080 Ti, except the two Glaive
scale runs, which were trained and evaluated on rented RTX 3090s while the
local GPU was unavailable. The base model is Qwen3-0.6B
...
zero-shot, 15.2\% 3-shot on the curated set). Latency is mean per-example,
fp16, including tokenization: GTX 1080 Ti for the curated row; the Glaive-100
row\textsuperscript{$\dagger$} was measured on an RTX 3090 (different
GPU---compare with care); the Glaive-200 row\textsuperscript{$\ddagger$} was
measured on another RTX 3090 host whose 2014-era CPU makes single-sample
latency launch-overhead bound---batch throughput is the informative figure
(see text). Macro-F1 is over the classes present
...
for future LoRA/transfer-learning work. Temperature is fit on each setting's
validation split only ($T=1.7912$ curated, $T=1.4063$ Glaive-100,
$T=1.3922$ Glaive-200). The
generative baselines for the Glaive-100 and Glaive-200 settings are pending
(---) and will complete the rows when run.}
  \label{tab:main}
  \begin{tabular}{lcc@{\hspace{1.2em}}cc@{\hspace{1.2em}}cc}
    ...
    Glaive-100, tool routing ($n{=}2{,}557$) & --- & \textbf{0.960} & --- & \textbf{0.880} & --- & \textbf{22.2}\textsuperscript{$\dagger$} \\
    Glaive-200, tool routing ($n{=}2{,}805$) & --- & \textbf{0.939} & --- & \textbf{0.845} & --- & \textbf{55.0}\textsuperscript{$\ddagger$} \\
    \bottomrule
  \end{tabular}
\end{table}
...
Scaling to a larger catalog preserves the picture. On the 101-class
Glaive-100 setting (100 tools + \texttt{no\_tool}, $n=2{,}557$), the same
frozen-Qwen3-0.6B classifier reaches \textbf{96.0\% accuracy / 0.880 macro-F1}
at \textbf{22.2\,ms} per decision. The accuracy drop from the 25-class setting
concentrates in the thin tail---26 of 101 classes have fewer than 40
post-deduplication training examples, with test supports as small as 1--3---and
in near-synonym confusions (\texttt{get\_recipe}$\to$\texttt{search\_recipes},
\texttt{get\_news}$\to$\texttt{get\_news\_articles}). Selective prediction at
confidence $\ge 0.9$ covers 88.7\% of queries at 99.0\% accuracy, rejecting
79 of the 101 test errors.

At 179 classes the trend continues. The Glaive-200 setting (178 tools +
\texttt{no\_tool}, $n=2{,}805$) reaches \textbf{93.9\% accuracy / 0.845
macro-F1}. The drop again concentrates in the thin tail---\texttt{add\_task},
\texttt{analyze\_customer\_reviews}, and \texttt{create\_account} all score
F1 $=0$ on tiny supports---and in near-synonym pairs
(\texttt{search\_movies}$\to$\texttt{get\_movie\_recommendations},
\texttt{get\_recipe}$\to$\texttt{search\_recipes},
\texttt{get\_news\_articles}$\to$\texttt{get\_news}, 6 confusions each).
Selective prediction at confidence $\ge 0.9$ holds 85.6\% coverage at 98.1\%
accuracy, rejecting 125 of the 171 test errors. Per-decision latency on the
rented 3090 host measured 55.0\,ms, but that figure is launch-overhead bound
by the host's 2014-era CPU and is not comparable across rows; batch
throughput---257 queries/s at batch 16, 1{,}055/s at batch 64---is the
informative number.

A natural objection is that routing needs no trained model at all---just
retrieve the most similar tool description. Table~\ref{tab:retrieval} tests
it: BM25 and TF-IDF over per-tool descriptions (names only for the curated
catalog, which ships none), with \texttt{no\_tool} predicted when the top
score falls below a threshold tuned on each validation split for macro-F1.
At 25 tools the untrained baselines are respectable---0.605 accuracy, ahead
of the 3-shot generative baseline's 0.512---but they collapse as the catalog
grows (0.605 $\to$ 0.447) while the trained head holds (0.993 $\to$ 0.939).
Near-synonym tools (\texttt{get\_recipe} vs.\ \texttt{search\_recipes})
collide lexically; the classifier learns the fine distinctions that
similarity cannot see.

\begin{table}[t]
  \centering
  \small
  \caption{Retrieval baselines vs.\ Sys One. BM25 and TF-IDF over per-tool
  descriptions (names only for the curated catalog); \texttt{no\_tool} via a
  score threshold tuned on each validation split for macro-F1. No training,
  CPU-only.}
  \label{tab:retrieval}
  \begin{tabular}{lcc@{\hspace{1.2em}}cc@{\hspace{1.2em}}cc}
    \toprule
    & \multicolumn{2}{c}{BM25} & \multicolumn{2}{c}{TF-IDF} & \multicolumn{2}{c}{Sys One} \\
    \cmidrule(lr){2-3} \cmidrule(lr){4-5} \cmidrule(lr){6-7}
    Setting & Acc. & Macro-F1 & Acc. & Macro-F1 & Acc. & Macro-F1 \\
    \midrule
    Curated ($n{=}1{,}641$) & 0.604 & 0.602 & 0.605 & 0.611 & \textbf{0.993} & \textbf{0.989} \\
    Glaive-100 ($n{=}2{,}557$) & 0.465 & 0.435 & 0.492 & 0.473 & \textbf{0.960} & \textbf{0.880} \\
    Glaive-200 ($n{=}2{,}805$) & 0.410 & 0.380 & 0.447 & 0.425 & \textbf{0.939} & \textbf{0.845} \\
    \bottomrule
  \end{tabular}
\end{table}
...
Latency wins compound into cost wins. At the \$0.1422/hr rental rate of the
RTX 3090 host, the 179-class router's batch-64 throughput (1{,}055
queries/s) prices routing at roughly \textbf{\$0.04 per million queries};
at single-query latency the same hardware is \$2.17 per million. The 3-shot
generative baseline (500.6\,ms per query on the 25-tool catalog) costs
\$19.77 per million at the same hourly rate---about $9\times$ the
single-query cost and $530\times$ the batched cost, on identical 0.6B
parameters. The gap widens structurally with catalog size: the generative
prompt must carry every tool definition, so its token count grows linearly
with $N$ ($\sim$1{,}550 tokens at 100 tools, $\sim$2{,}744 at 178, by a
chars/4 estimate over the measured descriptions), while the classifier's
per-query cost is $O(1)$ in $N$.
